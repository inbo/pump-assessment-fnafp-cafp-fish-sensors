...
# CRT for state OK 
form.state.df.tree <- as.formula(State ~ rpm + Type + Species + Length)
...
for(i in specieslist_OK[1]){
  k=k+1
  form.state.df.tree <- as.formula(State ~ rpm + Length + Type)
  model.state.df.tree[[k]] <- rpart(form.state.df.tree,df_OK_f[which(df_OK_f$Species==i),],control=control)
  b[[k]]<-fancyRpartPlot(model.state.df.tree[[k]],main=paste(i),box.palette=color.selection,palettes = "PuRd")
  b[[k]]
...
When using all the data, it becomes clear that if fish were eel there was a 0 percent chance of dying. After removing these data what remained was the bream and roach. The control samples of these fish had a 1 percent chance of dying After removing these data what remained was the bream and roach that passed the pump. If either a bream or roach were smaller than 198 mm they had a 30 percent chance of dying. In contrast, fish that were larger than 340 mm had 76 percent chance of dying. Bream that were smaller than 340 mm had 33 and 73 percent chance of dying if they went through the pump when the rpm was low versus high respectively. It should be noted that fish length and species were strongly correlated. Because of this it is difficult to disentangle whether the lower mortality of roach compared to bream is because of the lower length of the former (168 mm versus 374 mm), or species-specific behaviour and/or vulnerability. There were no roach larger than 198 mm and no bream smaller than 198 mm. Hence fish smaller than 198 mm would have been roach while fish larger than 198 mm would have been bream. Therefore the distinction based on length coincides with the distinction between species. 

When considering the species separately, no decision tree could be developed for neither roach nor eel. 
...
form.state.df.tree <- as.formula(Injury ~ Species + rpm + Type + Length)
...
for(i in "bream"){
  k=k+1
  form.state.df.tree <- as.formula(Injury ~ rpm + Type + Length)
