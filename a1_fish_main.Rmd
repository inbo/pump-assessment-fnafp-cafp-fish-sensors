...
df_OK_f %>% 
  ...
  kable(caption="Different scenarios and samples for OK")
```
...
df_OK_f %>% 
  ...
  count() %>% 
  kable(caption="Number of recaptured individuals per species for OK")
```
...
df_OK_f %>% 
  ...
  tally() %>% ungroup() %>% 
  kable(caption = "number of fish with different numbers of injuries per scenario for OK.")
```
