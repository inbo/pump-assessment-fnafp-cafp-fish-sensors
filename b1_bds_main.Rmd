...
file.final.P0 <- file.final %>%
  group_by(sensor) %>%
  filter((scenario=="CAFP" & P..hPa.>1050 & relative.to.injection > 50*0.01)| # 1050 mbar crossing for pressure, AFTER injection event
         (scenario=="FNAFP - 468 rpm" & P..hPa.>1050 & relative.to.injection > 50*0.01)| # 1050 mbar crossing for pressure, AFTER injection event
         (scenario=="FNAFP - 550 rpm" & P..hPa.>1050 & relative.to.injection > 50*0.01)| # 1050 mbar crossing for pressure, AFTER injection event
         (scenario=="FNAFP - 468 rpm Ctrl" & P..hPa.>1070 & relative.to.injection > 50*0.01)| # 1070 mbar crossing for pressure, AFTER injection event
         (scenario=="FNAFP - 550 rpm Ctrl" & P..hPa.>1070 & relative.to.injection > 50*0.01)) %>% # 1070 mbar crossing for pressure, AFTER injection event
...
file.final.P1 <- file.final %>%                                
  group_by(sensor) %>%
  filter((scenario=="CAFP" & relative.to.nadir>(-100*0.01) & relative.to.nadir<0)| # Pressure maxima between the moment of nadir and one second before the moment of nadir
         (scenario=="FNAFP - 468 rpm" & relative.to.nadir>(-100*0.01) & relative.to.nadir<0)| # Pressure maxima between the moment of nadir and one second before the moment of nadir
         (scenario=="FNAFP - 550 rpm" & relative.to.nadir>(-100*0.01) & relative.to.nadir<0)) %>% # Pressure maxima between the moment of nadir and one second before the moment of nadir
...
file.final.P2 <- file.final %>%                                
  group_by(sensor) %>%
  filter((scenario=="CAFP" & relative.to.nadir<(100*0.01) & relative.to.nadir>0)| #maxima immediately after blade nadir
         (scenario=="FNAFP - 468 rpm" & relative.to.nadir<(100*0.01) & relative.to.nadir>0)| # maxima immediately after blade nadir
         (scenario=="FNAFP - 550 rpm" & relative.to.nadir<(100*0.01) & relative.to.nadir>0)) %>% # maxima immediately after blade nadir
  filter(P..hPa.==max(P..hPa.)) %>% 
  filter(Time..s.==min(Time..s.)) %>%  
  slice(1) %>%
  ungroup() 
...
file.final.P3 <- file.final %>%                                
  group_by(sensor) %>%
  filter((scenario=="CAFP" & relative.to.P2<(100*0.01) & relative.to.P2>0)| # minima immediately after second local maxima (P2)
         (scenario=="FNAFP - 468 rpm" & relative.to.P2<(100*0.01) & relative.to.P2>0)| # minima immediately after second local maxima (P2)
         (scenario=="FNAFP - 550 rpm" & relative.to.P2<(100*0.01) & relative.to.P2>0)) %>% # minima immediately after second local maxima (P2)
  filter(P..hPa.==min(P..hPa.)) %>% 
  filter(Time..s.==min(Time..s.)) %>%  
  slice(1) %>%
  ungroup() 
...
file.final.P4 <- file.final %>%                                
  group_by(sensor) %>%
  filter((scenario=="CAFP" & relative.to.P3 < (1500*0.01) & relative.to.P3 > 0)| # Local maxima of the control region, as sensor passes into the cage / net
         (scenario=="FNAFP - 468 rpm" & relative.to.P3 < (1500*0.01) & relative.to.P3 > 0)| # Local maxima of the control region, as sensor passes into the cage / net
         (scenario=="FNAFP - 550 rpm" & relative.to.P3 < (1500*0.01) & relative.to.P3 > 0)) %>% # Local maxima of the control region, as sensor passes into the cage / net
  filter(P..hPa.==max(P..hPa.)) %>% 
  filter(Time..s.==min(Time..s.)) %>%  
  slice(1) %>%
  ungroup() 
...
file.final.P4_control <- file.final %>%                                
  group_by(sensor) %>%
  filter((scenario=="FNAFP - 468 rpm Ctrl" & relative.to.nadir < 2000*0.01 & relative.to.nadir > 0)| # Peak pressure after P0
         (scenario=="FNAFP - 550 rpm Ctrl" & relative.to.nadir < 2000*0.01 & relative.to.nadir > 0)) %>% # Peak pressure after P0
  filter(P..hPa.==max(P..hPa.)) %>% 
  filter(Time..s.==max(Time..s.)) %>%  
  slice(1) %>%
  ungroup() 
...
file.final.P5 <- file.final %>%                                
  group_by(sensor) %>%
  filter((scenario=="CAFP" & local_minima_P==TRUE & relative.to.P4>(1000*0.01))| # Pressure minima after peak, indicating the sensor is now in the cage, net or tailwater
         (scenario=="FNAFP - 468 rpm" & local_minima_P==TRUE & relative.to.P4>(1000*0.01))| # Pressure minima after peak, indicating the sensor is now in the cage, net or tailwater
         (scenario=="FNAFP - 550 rpm" & local_minima_P==TRUE & relative.to.P4>(1000*0.01))| # Pressure minima after peak, indicating the sensor is now in the cage, net or tailwater
         (scenario=="FNAFP - 468 rpm Ctrl" & local_minima_P==TRUE & relative.to.P4>(2000*0.01))| # Pressure minima after peak, indicating the sensor is now in the cage, net or tailwater
         (scenario=="FNAFP - 550 rpm Ctrl" & local_minima_P==TRUE & relative.to.P4>(2000*0.01))) %>% # Pressure minima after peak, indicating the sensor is now in the cage, net or tailwater
  filter(Time..s.==min(Time..s.)) %>%  
  slice(1) %>%
  ungroup() 
