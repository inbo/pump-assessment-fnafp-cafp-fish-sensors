...
# Define the threshold value for strain rate s-1
th<-500
  ...
  for (i in sensors[1:5]){
    ...
    #create plot with 2 lines one for Plc and one for Prc
     g <- ggplot(df_shear.sensor, aes(x = Time..s.)) +
            ...
            geom_hline(yintercept = th, linetype = "dashed", color = "black", aes(label = "U = 5 m/s")) +  # Threshold line 5 m/s
            theme_minimal() +
            ggtitle(paste0(s, "/", i)) +
            scale_color_manual(name = "Legend", values = c("straincr" = "red", "straincr" = "green","mean_strain" = "blue","th" = "black"))  # Custom colors for each line
    l[[k]]<-ggplotly(g)

      # Identify where strain exceeds the threshold
    above_threshold <- df_shear.sensor$mean_strain > th
    
    # Find the start and end of each "over-threshold" period
    diff_mean_strain <- c(FALSE, diff(above_threshold) != 0)  # Detect changes in the logical vector
    event_starts <- which(diff_mean_strain & above_threshold)
    event_ends <- which(diff_mean_strain & !above_threshold) - 1
    
    # Handle if the period continues till the last row
    if (length(event_starts) > length(event_ends)) {
      event_ends <- c(event_ends, nrow(df_shear.sensor))
    }
    ...
    for (j in 1:length(event_starts)) {
      ...
      # Get the time range
      start_time <- df_shear.sensor$Time..s.[start_idx]
      end_time <- df_shear.sensor$Time..s.[end_idx]
      duration <- end_time - start_time
      
      # Calculate average and maximum strain during the over-threshold period
      avg_mean_strain <- mean(df_shear.sensor$mean_strain[start_idx:end_idx], na.rm = TRUE)
      max_mean_strain <- max(df_shear.sensor$mean_strain[start_idx:end_idx], na.rm = TRUE)
