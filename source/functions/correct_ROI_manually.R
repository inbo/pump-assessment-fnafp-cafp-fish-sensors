correct_ROI_manually<-function(data,sensor.select,time.select,stage){
  ...
  if (file.exists("./data/bds/internal/Manually_changed_ROI.csv")==FALSE){
    write.csv(old_and_new_temp,"./data/bds/internal/Manually_changed_ROI.csv",row.names = FALSE)
  }
  if (file.exists("./data/bds/internal/Manually_changed_ROI.csv")==TRUE){
    old_and_new<-read.csv("./data/bds/internal/Manually_changed_ROI.csv")
    old_and_new<-rbind(old_and_new,old_and_new_temp)
    write.csv(old_and_new,"./data/bds/internal/Manually_changed_ROI.csv",row.names = FALSE)
  }
